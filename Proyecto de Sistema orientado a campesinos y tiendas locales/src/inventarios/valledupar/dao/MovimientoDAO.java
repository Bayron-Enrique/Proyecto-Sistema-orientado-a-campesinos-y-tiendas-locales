...
public class MovimientoDAO implements IMovimientoDAO {
    ...
    public void guardar(Movimiento movimiento) {
        ...
        String sql = "INSERT INTO movimientos (id_producto, id_usuario, tipo_movimiento, cantidad, fecha_movimiento, observacion, stock_resultante, activo, fecha_creacion) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?, 1, SYSDATE)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setDate(5, movimiento.getFechaMovimiento());
    ...
    public List<Movimiento> buscarTodos() {
        ...
        String sql = "SELECT * FROM movimientos ORDER BY id_movimiento";
    ...
    public List<Movimiento> buscarPorProducto(int idProducto) {
        List<Movimiento> lista = new ArrayList<>();
        String sql = "SELECT * FROM movimientos WHERE id_producto = ? ORDER BY id_movimiento";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Movimiento> buscarPorPeriodo(String fechaInicio, String fechaFin) {
        List<Movimiento> lista = new ArrayList<>();
        String sql = "SELECT * FROM movimientos WHERE fecha_movimiento BETWEEN TO_DATE(?, 'YYYY-MM-DD') AND TO_DATE(?, 'YYYY-MM-DD') + 1 ORDER BY id_movimiento";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
